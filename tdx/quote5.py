#!/usr/bin/env python3
# 持仓盘口滑点表：读客户端持仓 → 批量取 5档 → 相对 micro price 算滑点。
# 6 档名义金额：1W买/1W卖/5W买/5W卖/10W买/10W卖。
# 5档吃不满的，按已成交部分算滑点并加 ">" 表示实际更差。
# 依赖：pip install pytdx；客户端需登录交易并点过一次持仓查询。
...
def parse(block):
    ...
    positions = []
    for ln in lines:
        if not re.match(r"\d{6}\|", ln): continue
        f = ln.split("|")
        nf = len(f)
        if nf < 15: continue
        if nf >= 22:
        ...
        def get(i): return num(f[i]) if i < nf else None
...
# 现价 ±1% / ±2% 的价格档位
PCT_LEVELS = [("+1%", 1.01), ("-1%", 0.99), ("+2%", 1.02), ("-2%", 0.98)]
...
def main():
    ...
    quotes = api.get_security_quotes(req) or []
    api.disconnect()
    qmap = {(q["market"], q["code"]): q for q in quotes}
    ...
    for ci in range(n_cols):
        ...
        for i in range(len(rows)):
            if not rows[i][1]:
                continue
            v, disp = rows[i][1][ci]
            code = "" if v is None else ("32" if abs(
                v) < 10 else ("33" if abs(v) < 20 else "31"))
